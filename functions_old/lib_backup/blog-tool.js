...
<methodCall>
  ...
  // parseGPTResponseメソッドの修正版
// HTMLヘッダーを正しく処理してタイトルを抽出
...
parseGPTResponse(content, category) {
  ...
  // HTMLのheadタグからタイトルを抽出する処理を追加
  let title = null;
  let bodyContent = content;
  
  // 1. HTMLのtitleタグからタイトルを取得（最優先）
  ...
  // 3. 不要なHTMLタグ（DOCTYPE, html, head）を削除
  ...
  // 6. それでもタイトルがない、または短すぎる場合はデフォルト生成
  if (!title || title.length < 10 || title === '導入') {
...
<p>${paragraphs[paragraphs.length - 1]}</p>`;
  ...
  // generateWithGPTメソッドも修正（HTMLヘッダーを生成しないように指示）
